...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            ...
            <div class="actions">
                <a href="{{ URL::to('/accomplishedOrder'.Helper::queryPageStr($qpArr)) }}" class="btn btn-sm blue-dark">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('change', '#productId', function (e) {
            ...
            $('#showGrade').html('');
            $.ajax({
                url: "{{ URL::to('accomplishedOrder/rwBreakdownGetBrand')}}",
                type: "POST",
                dataType: "json",
                headers: {
        ...
        $(document).on('change', '#brandId', function (e) {
            ...
            $('#showRwBreakdownView').html('');
            $.ajax({
                url: "{{ URL::to('accomplishedOrder/rwBreakdownGetGrade')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on('change', '#gradeId', function (e) {
            ...
            $('#showRwBreakdownView').html('');
            $.ajax({
                url: "{{ URL::to('accomplishedOrder/getRwBreakdownView')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
// Serialize the form data
            ...
            $.ajax({
                url: "{{ URL::to('accomplishedOrder/rwProceedRequest') }}",
                type: "POST",
...
// Serialize the form data
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    setTimeout(
                            window.location.replace('{{ route("accomplishedOrder.index")}}'
                                    ), 7000);
                },
                error: function (jqXhr, ajaxOptions, thrownError) {
...
// Serialize the form data
            var formData = new FormData($('#proceedForm')[0]);
            $.ajax({
                url: "{{ URL::to('accomplishedOrder/rwProceedRequestEdit') }}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                cache: false,
                contentType: false,
...
// Serialize the form data
            ...
            $.ajax({
                url: "{{ URL::to('accomplishedOrder/rwPreviewRequest') }}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                cache: false,
                contentType: false,
...
// Serialize the form data
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ URL::to('accomplishedOrder/rwBreakDownSave') }}",
                        type: "POST",
                        dataType: 'json', // what to expect back from the PHP script, if anything
                        cache: false,
                        contentType: false,
...
// Serialize the form data
            ...
            $.ajax({
                url: "{{ URL::to('accomplishedOrder/getLeadRwParametersName') }}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                data: {
                    rw_unit_id: rwParaId,
