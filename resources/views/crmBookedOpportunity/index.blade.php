...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-list"></i>@lang('label.BOOKED_OPPORTUNITY_LIST')
            </div>
        </div>
        <div class="portlet-body">
            <div class="row">
                <!-- Begin Filter-->
                {!! Form::open(array('group' => 'form', 'url' => 'crmBookedOpportunity/filter','class' => 'form-horizontal')) !!}
                <div class="row">
                    <div class="col-md-12">
                        <div class="col-md-4">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center vcenter">
                                ...
                                <span class="label label-sm label-blue-steel">@lang('label.PENDING')</span>
                                @elseif($target->dispatch_status == '1')
                                @if($target->approval_status == '0')
                                <span class="label label-sm label-purple">@lang('label.DISPATCHED')</span>
                                ...
                                <span class="label label-sm label-red-soft">@lang('label.DENIED')</span>
                                @endif
                                @endif
                            </td>
                            <td class="td-actions text-center vcenter">
                                <div class="width-inherit">
                                    @if($target->dispatch_status == '0' || $target->approval_status == '2')
                                    @if(!empty($userAccessArr[72][27]))
                                    <button class="btn btn-xs purple tooltips dispatch-opportunity vcenter" data-toggle="modal" title="@lang('label.CLICK_TO_DISPATCH')" data-id ="{{$target->id}}">
                                        <i class="fa fa-paper-plane"></i>
                                    </button>
                                    @endif
                                    @endif
                                    @if(!empty($userAccessArr[72][5]))
                        ...
                        @else
                        <tr>
                            <td colspan="10">@lang('label.NO_BOOKED_OPPORTUNITY_FOUND')</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
...
<script type="text/javascript">
    $(function () {
        ...
        //START:: Ajax for Dispatch Opportunity
        $(document).on("click", ".dispatch-opportunity", function (e) {
            e.preventDefault();
            ...
            swal({
                title: 'Are you sure you want to Dispatch this Opportunity?',
                type: 'warning',
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ URL::to('crmBookedOpportunity/dispatch')}}",
                        type: 'POST',
                        dataType: 'json',
                        data: {
                        ...
                        success: function (res) {
                            ...
                            setTimeout(
                                    window.location.replace('{{ route("crmBookedOpportunity.index")}}'),
                                    3000);
                        },
        ...
        //END:: Ajax for Dispatch Opportunity
        ...
        $(".opportunity-details").on("click", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('/crmBookedOpportunity/getOpportunityDetails')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("click", "#submitDenyOpportunity", function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ URL::to('crmBookedOpportunity/deny') }}",
                        type: "POST",
        ...
        $(".set-activity-log").on("click", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('crmBookedOpportunity/getOpportunityActivityLogModal')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
