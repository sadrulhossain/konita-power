...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            ...
            <div class="actions">
                <a href="{{ URL::to('/pendingOrder'.Helper::queryPageStr($qpArr)) }}" class="btn btn-sm blue-dark">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('change', '#productId', function (e) {
            ...
            $('#showGrade').html('');
            $.ajax({
                url: "{{ URL::to('pendingOrder/rwBreakdownGetBrand')}}",
                type: "POST",
                dataType: "json",
                headers: {
        ...
        $(document).on('change', '#brandId', function (e) {
            ...
            $('#showRwBreakdownView').html('');
            $.ajax({
                url: "{{ URL::to('pendingOrder/rwBreakdownGetGrade')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on('change', '#gradeId', function (e) {
            ...
            $('#showRwBreakdownView').html('');
            $.ajax({
                url: "{{ URL::to('pendingOrder/getRwBreakdownView')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
// Serialize the form data
            ...
            $.ajax({
                url: "{{ URL::to('pendingOrder/rwProceedRequest') }}",
                type: "POST",
...
// Serialize the form data
            var formData = new FormData($('#proceedForm')[0]);
            $.ajax({
                url: "{{ URL::to('pendingOrder/rwProceedRequestEdit') }}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                cache: false,
                contentType: false,
...
// Serialize the form data
            ...
            $.ajax({
                url: "{{ URL::to('pendingOrder/rwPreviewRequest') }}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                cache: false,
                contentType: false,
...
// Serialize the form data
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ URL::to('pendingOrder/rwBreakDownSave') }}",
                        type: "POST",
                        dataType: 'json', // what to expect back from the PHP script, if anything
                        cache: false,
                        contentType: false,
...
// Serialize the form data
            ...
            $.ajax({
                url: "{{ URL::to('pendingOrder/getLeadRwParametersName') }}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                data: {
                    rw_unit_id: rwParaId,
