...
<div class="col-md-12">
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-exchange"></i>@lang('label.TRANSFER_BUYER_TO_SALES_PERSON')
            </div>
        </div>
        <div class="portlet-body form">
            {!! Form::open(array('group' => 'form', 'url' => '', 'class' => 'form-horizontal', 'id' => 'transferBuyerToSalesPersonForm')) !!}
            {{csrf_field()}}
            <div class="form-body">
                <div class="row">
...
<?php } ?>
        ...
        $(document).on('change', '#salesPersonId', function () {
            ...
            $.ajax({
                url: '{{URL::to("transferBuyerToSalesPerson/getBuyersToRelate/")}}',
                type: 'POST',
                dataType: 'json',
                headers: {
        ...
        $(document).on("click", "#relateProduct", function (e) {
            ...
            var salesPersonId = $("#salesPersonId").val();
            $.ajax({
                url: "{{ URL::to('/transferBuyerToSalesPerson/getRelatedProducts')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                closeOnCancel: true},
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $.ajax({
                                url: "{{URL::to('transferBuyerToSalesPerson/relateSalesPersonToBuyer')}}",
                                type: "POST",
        ...
        $(document).on("click", "#getSalesPersonToTransfer", function (e) {
            ...
            $.ajax({
                url: "{{URL::to('transferBuyerToSalesPerson/getSalesPersonToTransfer')}}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                cache: false,
                contentType: false,
