...
<div class="col-md-12">
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-industry"></i>@lang('label.RELATE_BUYER_TO_PRODUCT')
            </div>
        </div>
        <div class="portlet-body form">
            {!! Form::open(array('group' => 'form', 'url' => '', 'class' => 'form-horizontal', 'id' => 'buyerToProductRelateForm')) !!}
            {{csrf_field()}}
            <div class="form-body">
                <div class="row">
                    <div class="col-md-7">
                            ...
                            <div class="row relation-div-box-default">
                                <div class="col-md-12 border-bottom-1-green-seagreen">
                                    <h5>
                                        ...
                                        @if(!empty($assignedProductList) && empty($dependentProductArr[$request->get('buyer_id')]))
                                        <a class="btn btn-xs bold btn-circle red-intense pull-right remove-all-assignments tooltips " title="@lang('label.CLICK_TO_REMOVE_ALL')" 
                                           ...
                                           data-buyer-id="{{ $request->get('buyer_id') }}">
                                        ...
                                        <table class="table">
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if(!empty($dependentProductArr[$request->get('buyer_id')]) && in_array($productId, $dependentProductArr[$request->get('buyer_id')]))
                            ...
                            <div class="row margin-bottom-10">
                                <div class="col-md-12">
                                    ...
                                    <button class='label label-primary tooltips' href="#modalRelatedProduct" id="relateProduct"  data-toggle="modal" title="@lang('label.SHOW_RELATED_PRODUCTS')">
                                        @lang('label.PRODUCT_RELATED_TO_THIS_BUYER'): {!! !empty($productRelatedToBuyer) ? count($productRelatedToBuyer):0 !!}&nbsp; <i class="fa fa-search-plus"></i>
                                    </button>
                                    @endif
                                </div>
                            </div>
                                        ...
                                        <table class="table table-bordered table-hover relation-view">
                                            <thead>
                                                <tr class="active">
                                                    ...
                                                    <?php
                                                    $allCheckDisabled = '';
                                                    if (!empty($dependentBrandArr[$request->get('buyer_id')][$request->get('product_id')])) {
                                            ...
                                            <tbody>
                                                ...
                                                $checked = '';
                                                if (!empty($brandRelatedToBuyer[Request::get('product_id')]) && array_key_exists($brandId, $brandRelatedToBuyer[Request::get('product_id')])) {
                                                    $checked = 'checked';
                                                }
                                                ...
                                                if (!empty($dependentBrandArr[$request->get('buyer_id')][$request->get('product_id')])) {
                                                    if (in_array($brandId, $dependentBrandArr[$request->get('buyer_id')][$request->get('product_id')]) && ($checked != '')) {
                                                        $brandDisabled = 'disabled';
                                                        $checkCondition = 1;
                            ...
                            <div class="form-actions">
                                <div class="row">
                                    <div class="col-md-offset-4 col-md-8">
                                        @if(!empty($relatedBrandArr[Request::get('product_id')]))
                                        @if(!empty($userAccessArr[22][7]))
                                        <button class="btn btn-circle green btn-submit" id="saveBuyerToProductRel" type="button">
                                            <i class="fa fa-check"></i> @lang('label.SUBMIT')
                                        </button>
                                        @endif
                                        @if(!empty($userAccessArr[22][1]))
                                        <a href="{{ URL::to('/buyerToProduct') }}" class="btn btn-circle btn-outline grey-salsa">@lang('label.CANCEL')</a>
                                        @endif
                                        @endif
                                    </div>
                                </div>
...
<?php } ?>
        ...
        $(document).on('change', '#buyerId', function () {
            ...
            $.ajax({
                url: '{{URL::to("buyerToProduct/getAssignedProducts/")}}',
                type: 'POST',
                dataType: 'json',
                headers: {
        ...
        $(document).on('change', '#buyerId, #productId', function () {
            var buyerId = $('#buyerId').val();
            var productId = $('#productId').val();
            ...
            $.ajax({
                url: '{{URL::to("buyerToProduct/getProductsToRelate/")}}',
                type: 'POST',
                dataType: 'json',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("click", "#relateProduct", function (e) {
            ...
            var buyerId = $("#buyerId").val();
            $.ajax({
                url: "{{ URL::to('/buyerToProduct/getRelatedProducts')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                closeOnCancel: true},
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            var form_data = new FormData($('#buyerToProductRelateForm')[0]);
                            $.ajax({
                                url: "{{URL::to('buyerToProduct/relateBuyerToProduct')}}",
                                type: "POST",
                                ...
                                success: function (res) {
                                    ...
                                    var buyerId = $('#buyerId').val();
                                    var productId = $('#productId').val();
                                    location = "buyerToProduct?buyer_id=" + buyerId + "&product_id=" + productId;
                                },
                                error: function (jqXhr, ajaxOptions, thrownError) {
                                    if (jqXhr.status == 400) {
                                        var errorsHtml = '';
                ...
                closeOnCancel: true},
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $.ajax({
                                url: "{{URL::to('buyerToProduct/removeAssignedProduct')}}",
                                type: "POST",
                                dataType: 'json', // what to expect back from the PHP script, if anything
                                ...
                                success: function (res) {
                                    toastr.success(res.message, res.heading, options);
                                    location = "buyerToProduct?buyer_id=" + buyerId + "&product_id=" + selectedProductId;
                                },
                                error: function (jqXhr, ajaxOptions, thrownError) {
                                    if (jqXhr.status == 400) {
                                        var errorsHtml = '';
                ...
                closeOnCancel: true},
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            $.ajax({
                                url: "{{URL::to('buyerToProduct/removeAllAssignment')}}",
                                type: "POST",
                                dataType: 'json', // what to expect back from the PHP script, if anything
