            ...
            <table class="table table-bordered table-hover relation-view">
                <thead>
                    <tr class="active">
                        ...
                        <?php
                        $allCheckDisabled = '';
                        if (!empty($dependentBrandArr[$request->buyer_id][$request->product_id])) {
                ...
                <tbody>
                    ...
                    $checked = '';
                    if (!empty($brandRelatedToBuyer[$request->product_id]) && array_key_exists($brandId, $brandRelatedToBuyer[$request->product_id])) {
                        $checked = 'checked';
                    }
                    ...
                    if (!empty($dependentBrandArr[$request->buyer_id][$request->product_id])) {
                        if (in_array($brandId, $dependentBrandArr[$request->buyer_id][$request->product_id]) && ($checked != '')) {
                            $brandDisabled = 'disabled';
                            $checkCondition = 1;
...
//        $(".pr-check").each(function () {
//            var productId = $(this).val();
//            if ($("#pr_" + productId).prop("checked")) {
//                $(".br-check-" + productId).prop("disabled", false);
//            } else {
//                $(".br-check-" + productId).prop("disabled", true);
//            }
//        });
//
//        $(".pr-check").on("click", function () {
//            var productId = $(this).val();
//            if ($(this).prop('checked')) {
//                $(".br-check-" + productId).prop("disabled", false);
//            } else {
//                $(".br-check-" + productId).prop("disabled", true);
//            }
//            if ($('.pr-check:checked').length == $('.pr-check').length) {
...
//            if ($(this).prop('checked')) {
//                $('.pr-check').prop("checked", true);
//                $('.brand-check').prop("disabled", false);
//            } else {
//                $('.pr-check').prop("checked", false);
//                $('.brand-check').prop("disabled", true);
//            }
//
//        });
//        if ($('.pr-check:checked').length == $('.pr-check').length) {
