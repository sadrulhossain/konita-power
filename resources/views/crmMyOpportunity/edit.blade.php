...
<div class="col-md-12">
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-list"></i>@lang('label.EDIT_MY_OPPORTUNITY')
            </div>
        </div>
        <div class="portlet-body form">
            {!! Form::open(array('group' => 'form', 'url' => '', 'id' => 'myOppornunityCreatForm','class' => 'form-horizontal')) !!}
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                ...
                                <span class="buyer-select-span">
                                    {!! Form::select('buyer_id', $buyerList, $buyerId, ['id'=> 'buyerId', 'class' => 'form-control buyer-select js-source-states' ]) !!}
                                </span>
            ...
            <div class="form-actions">
                <div class="row">
                    <div class="col-md-12 text-center">
                        ...
                        <a href="{{ URL::to('/crmMyOpportunity'.Helper::queryPageStr($qpArr)) }}" class="btn btn-circle btn-outline grey-salsa">@lang('label.CANCEL')</a>
                    </div>
                </div>
            </div>
            {!! Form::close() !!}
...
<script>
    $(function () {
        ...
        $(document).on("click", "#crmOpportunityEditSubmit", function (e) {
            ...
            var formData = new FormData($('#myOppornunityCreatForm')[0]);

            $.ajax({
                url: "{{route('crmMyOpportunity.update')}}",
                type: "POST",
                dataType: 'json', // what to expect back from the PHP script, if anything
                cache: false,
                contentType: false,
    ...
    $(document).on('change', '#buyerId', function (e) {
        ...
        $('#productWiseBrandId_' + productKey).html("<select class='form-control brand-select js-source-states brand-item'><option value='0'>@lang('label.SELECT_BRAND_OPT')</option></select>");
        ...
        $.ajax({
            url: "{{ URL::to('crmMyOpportunity/getBuyerContPerson')}}",
            type: "POST",
            dataType: "json",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
