...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-user"></i>@lang('label.BUYER_PAYMENT_VOUCHER')
            </div>
        </div>
        <div class="portlet-body">
            <div class="row">
                <div class="col-md-12">
                    <!-- Begin Filter-->
                    {!! Form::open(array('group' => 'form', 'url' => 'buyerPaymentVoucher/filter','class' => 'form-horizontal')) !!}
                    <div class="col-md-4">
                        ...
                        <table class="table table-bordered table-hover" id="fixedHeadTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center vcenter" rowspan="2">@lang('label.PAYMENT_DATE')</th>
                                    <th class="text-center vcenter" rowspan="2">@lang('label.TOTAL_BALANCE')</th>
                                    <th class="text-center vcenter" rowspan="2">@lang('label.PAID_AMOUNT')</th>
                                    <th class="text-center vcenter" rowspan="2">@lang('label.NET_DUE')</th>
                                    <th class="text-center vcenter" colspan="2">@lang('label.BUYER_CONTACT_PERSON')</th>
                                    <th class="text-center vcenter" rowspan="2">@lang('label.REMARKS')</th>
                                    <th class="text-center vcenter" rowspan="2">@lang('label.APPROVAL_STATUS')</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-left vcenter">{!! $target->buyer_contact_person ?? '' !!}</td>
                                    <td class="text-left vcenter">{!! $target->buyer_contact_number ?? '' !!}</td>
                                    <td class="text-left vcenter">{!! $target->remarks ?? '' !!}</td>
                                    ...
                                    <td class="td-actions text-center vcenter">
                                        <div class="width-inherit">
                                            ...
                                            <a class="btn btn-xs blue-soft tooltips vcenter" target="_blank" href="{{ URL::to('buyerPaymentVoucher/voucherPrint?payment_id=' . $target->id) }}"  title="@lang('label.PRINT')">
                                                <i class="fa fa-print"></i>
                                            </a>
                                            @endif
                                            @if($target->approval_status == '0')
                                            @if(!empty($userAccessArr[64][25]))
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".approve", function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ URL::to('buyerPaymentVoucher/approve')}}",
                        type: 'POST',
                        dataType: 'json',
                        data: {
        ...
        $(document).on("click", ".deny", function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ URL::to('buyerPaymentVoucher/deny')}}",
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            payment_id: paymentId,
