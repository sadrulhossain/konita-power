...
<div class="col-md-12">
    ...
    <div class="portlet box green">
        <div class="portlet-title">
            <div class="caption">
                <i class="fa fa-list"></i>@lang('label.CONFIRMED_ORDER_LIST')
            </div>
        </div>
        <div class="portlet-body">
            <!-- Begin Filter-->
            {!! Form::open(array('group' => 'form', 'url' => 'confirmedOrder/filter','class' => 'form-horizontal')) !!}
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        ?>
                        <tr>
                            <td class="text-center vcenter" rowspan="{{$rowSpan['inquiry']}}">{!! ++$sl.$iconCAS.$iconFH.$iconMsg !!}</td>
                            ...
                            <td class="td-actions text-center vcenter" rowspan="{{$rowSpan['inquiry']}}">
                                <div class="width-inherit">
                                    ...
                                    @if(!empty($userAccessArr[27][3]))
                                    @if(empty($deliveryArr[$target->id]) || sizeof($deliveryArr[$target->id]) == 1)
                                    <button class="btn btn-xs btn-primary edit-lc-info tooltips vcenter" href="#modalEditLcInfo" data-id="{!! $target->id !!}" data-toggle="modal" data-placement="top" data-rel="tooltip" title="Edit LC Information">
                                    ...
                                    @if(in_array($target->order_status, ['2','3']))
                                    <a class="btn btn-xs purple-sharp tooltips vcenter" href="{{ URL::to('confirmedOrder/getShipmentInfoView/' . $target->id) }}" data-placement="top" data-rel="tooltip" title="Set Shipment Information">
                                        <i class="fa fa-shopping-cart"></i>
                                    </a>
                                    @endif
                                    @endif

                                    @if(!empty($userAccessArr[27][14]))
                                    @if(in_array($target->order_status, ['3']))
                                    <button class="btn btn-xs green-seagreen mark-order-accomplished tooltips vcenter" href="#modalMarkOrderAccomplished" data-id="{!! $target->id !!}" data-toggle="modal" data-placement="top" data-rel="tooltip" title="Mark Order as Accomplished">
                                        <i class="fa fa-check-circle"></i>
                                    </button>
                                    @endif
                                    @endif
                                    @if(!empty($userAccessArr[27][13]))
                                    ...
                                    <button class="btn btn-xs purple-sharp tooltips vcenter order-messaging" title="@lang('label.VIEW_MESSAGES')" href="#modalOrderMessaging" data-id="{!! $target->id !!}" data-buyer-id="{!! $target->buyer_id !!}" data-toggle="modal">
                                    ...
                                    <!--RW BREAKDOWN--> 
                                    @if(!empty($userAccessArr[27][19]))
                                    @if(!empty($rwBreakdownStatusArr[$target->id]))
                                    <a class="btn btn-xs green-soft tooltips vcenter" title="@lang('label.RW_BREAKDOWN_EDIT')" href="{{ URL::to('confirmedOrder/rwBreakdown/' . $target->id . Helper::queryPageStr($qpArr ?? '')) }}">
                                    ...
                                    @else
                                    <a class="btn btn-xs yellow tooltips vcenter" title="@lang('label.RW_BREAKDOWN')" href="{{ URL::to('confirmedOrder/rwBreakdown/' . $target->id . Helper::queryPageStr($qpArr ?? '')) }}">
                                        <i class="fa fa fa-external-link"></i>
                                    </a>
                                    @endif
                                    @endif
                                    <!--ENDOF RW BREAKDOWN--> 
                                    <!--PO Generate-->
                                    @if(!empty($userAccessArr[27][20]))
                                    @if(!empty($rwBreakdownStatusArr[$target->id]))
                                    <a class="btn btn-xs btn blue-sharp tooltips vcenter" title="@lang('label.PO_GENERATE')" href="{{ URL::to('confirmedOrder/poGenerate/' . $target->id) }}">
                                    ...
                                    <!--PI GENERATE-->
                                    @if(!empty($userAccessArr[27][22]))
                                    @if(!empty($rwBreakdownStatusArr[$target->id]))
                                    @if($target->pi_required == '1')
                                    <a class="btn btn-xs btn blue-hoki tooltips vcenter" title="@lang('label.PI_GENERATE')" href="{{ URL::to('confirmedOrder/piGenerate/' . $target->id) }}">
...
<script type="text/javascript">
    $(function () {
        ...
        $(".order-messaging").on("click", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('/confirmedOrder/getOrderMessaging')}}",
                type: "POST",
                dataType: "json",
                headers: {
...
//            e.preventDefault();
            ...
            $.ajax({
                url: "{{ URL::to('confirmedOrder/setMessage')}}",
                type: 'POST',
                cache: false,
                contentType: false,
                processData: false,
        ...
        $(".edit-lc-info").on("click", function (e) {
            ...
            var inquiryId = $(this).attr('data-id');
            $.ajax({
                url: "{{ URL::to('confirmedOrder/edit')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(".order-cancel").on("click", function (e) {
            ...
            var inquiryId = $(this).attr('data-id');
            $.ajax({
                url: "{{ URL::to('confirmedOrder/orderCancellationModal')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(".order-details").on("click", function (e) {
            ...
            var inquiryId = $(this).attr("data-id");
            $.ajax({
                url: "{{ URL::to('/confirmedOrder/getOrderDetails')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(".mark-order-accomplished").on("click", function (e) {
            ...
            var inquiryId = $(this).attr('data-id');
            $.ajax({
                url: "{{ URL::to('confirmedOrder/markOrderAccomplishedModal')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("click", ".followup-history", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('confirmedOrder/getFollowUpModal')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("click", "#saveHistory", function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ URL::to('confirmedOrder/setFollowUpSave')}}",
                        type: 'POST',
                        cache: false,
                        contentType: false,
                        processData: false,
        ...
        $(document).on("click", ".commission-setup-modal", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('confirmedOrder/getCommissionSetupModal')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("click", "#cmsnSaveBtn", function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        url: "{{ URL::to('confirmedOrder/commissionSetupSave')}}",
                        type: 'POST',
                        cache: false,
                        contentType: false,
                        processData: false,
        ...
        $(".lsd-info").on("click", function (e) {
            ...
            var inquiryId = $(this).attr("data-id");
            $.ajax({
                url: "{{ URL::to('/confirmedOrder/lsdInfo')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("click", ".rw-breakdown-view", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('confirmedOrder/leadRwBreakdownView')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(document).on("click", "#quantitySummary", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('confirmedOrder/quantitySummaryView')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(".shipment-details").on("click", function (e) {
            ...
            $.ajax({
                url: "{{ URL::to('/confirmedOrder/getShipmentDetails')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $(".lead-time").on("click", function (e) {
            ...
            var inquiryId = $(this).attr("data-id");
            $.ajax({
                url: "{{ URL::to('/confirmedOrder/getLeadTime')}}",
                type: "POST",
                dataType: "json",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
