            ...
            <table class="table table-bordered table-hover relation-view">
                <thead>
                    <tr class="active">
                        ...
                        <?php
                        $allCheckDisabled = '';
                        if (!empty($dependentBrandArr[$request->sales_person_id][$request->product_id])) {
                ...
                <tbody>
                    ...
                    $checked = '';
                    if (!empty($brandRelatedToSalesPerson[$request->product_id]) && array_key_exists($brandId, $brandRelatedToSalesPerson[$request->product_id])) {
                        $checked = 'checked';
                    }
                    ...
                    if (!empty($dependentBrandArr[$request->sales_person_id][$request->product_id])) {
                        if (in_array($brandId, $dependentBrandArr[$request->sales_person_id][$request->product_id]) && ($checked != '')) {
                            $brandDisabled = 'disabled';
                            $checkCondition = 1;
